...
export const AuthProvider = ({ children }) => {
  ...
  const login = async (credentials) => {
    try {
      const response = await fetch('https://hr-backend-h02e.onrender.com/api/auth/login', {
        method: 'POST',
  ...
  const signup = async (userData) => {
    try {
      const response = await fetch('https://hr-backend-h02e.onrender.com/api/auth/signup', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
